...
# Upstream sets `disable-model-invocation: true` here. pi (0.84.x) silently drops
# any skill carrying that key instead of just hiding it from the model, so the
# skill would be unusable there. Removed deliberately: the description above
# already scopes invocation to an explicit /i-have-adhd request.
license: MIT
metadata:
